...
<div class="playlist_box" oncontextmenu="event.preventDefault()">
	<div style=";margin-bottom:8px;position:relative">
		...
		<a href="/watch?v=<?= $Playlist_Videos[$NextVideo]["url"] ?>&pl=<?= urlencode($_GET["pl"]) ?>" style="font-weight:bold;color:#222222;text-decoration:none;position:absolute;right:2px;top:7px;font-size:18px">NEXT</a>
		<? endif ?>
	</div>
	<div id="plcontainer">
		<? foreach($Playlist_Videos as $PVideo) : ?>
		<a href="/watch?v=<?= $PVideo["url"] ?>&pl=<?= urlencode($_GET["pl"]) ?>"<? if ($PVideo["url"] == $URL) : ?> style="background:#dddddd" id="plwatching"<? endif ?>>
