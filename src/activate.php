...
if (!$_USER->logged_in) {
    if (!isset($_GET["code"])) {
        ...
        notification("Please log in to activate your account!", "/login?activate=" . $_GET["code"], "red");
        exit();
    }
}
if ($_USER->Is_Activated) {
    notification("You've already activated your account!", "/", "red");
    exit();
}
if (!isset($_GET["code"])) {
    redirect("/");
...
$DB->modify(
    "DELETE FROM activations WHERE username = :USERNAME AND secret = BINARY :CODE",
    [
