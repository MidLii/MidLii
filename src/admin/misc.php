...
if ($_USER->logged_in && ($_USER->Is_Admin || $_USER->Is_Mod) && isset($_SESSION["admin_panel"])) {
    ...
    if (isset($_POST["save_logo"])) {
        ...
        if ($_POST["logo"] == 0) {
            ...
            notification("Logo changed to default!", "/admin/misc", "green");
            exit();
        } else {
            if (isset($_FILES["logo_file"])) {
                ...
                if ($Uploader->processed) {
                    ...
                    notification("Logo changed!", "/admin/misc", "green");
                    exit();
                }
            } else {
                notification("Nothing changed!", "/admin/misc", "red");
                exit();
            }
        }
    }
    ...
    if (isset($_POST["save_pages"])) {
        ...
        notification("Changed!", "/admin/misc", "green");
        exit();
    }

    if (isset($_POST["save_text"])) {
        if (mb_strpos($_POST["guidelines"], "<script>") !== false) {
            notification("You cannot use scripts!", "/admin/misc");
            exit();
        }
        $Guidelines = $_POST["guidelines"];

        if (mb_strpos($_POST["help"], "<script>") !== false) {
            notification("You cannot use scripts!", "/admin/misc");
            exit();
        }
        $Help = $_POST["help"];
