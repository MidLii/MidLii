...
if ($_USER->logged_in && ($_USER->Is_Admin || $_USER->Is_Mod) && isset($_SESSION["admin_panel"])) {
	...
	if (!isset($_GET["u"]) && !isset($_GET["d"])) {
		...
		if (isset($_GET["a"]) && ctype_alnum($_GET["a"])) {
			...
			if ($DB->RowNum == 1) {
				...
				if ($DB->RowNum == 1) {
					...
					$DB->modify(
						"INSERT INTO private_messages (from_user,to_user,message,subject,date_sent) VALUES ('admin',:USERNAME,:MESSAGE,'Partnership',NOW())",
						[
							":USERNAME" => $_GET["a"],
							":MESSAGE" => $Message
						]
					...
					notification("<strong>" . $_GET["a"] . "</strong> is now a VidLii Partner!", "/admin/users", "green");
					exit();
				}
			}
		}
		...
		if ($DB->RowNum == 1) {
			...
			$DB->modify(
				"INSERT INTO private_messages (from_user,to_user,message,subject,date_sent) VALUES ('admin',:USERNAME,:MESSAGE,'Partnership denied',NOW())",
				[
					":USERNAME" => $_GET["n"],
					":MESSAGE" => $Message
				]
			...
			notification("<strong>" . $_GET["n"] . "</strong>'s application has been denied!", "/admin/users", "green");
			exit();
		}
		...
		if ($DB->RowNum > 0) {
			...
			if ($Edit_User->Is_Admin) {
				notification("You cannot edit admins!", "/admin/users", "red");
				exit();
			}
			...
			if (isset($_POST["delete_avatar"])) {
				...
				if ($DB->RowNum == 1) {
					...
					notification("Avatar successfully deleted!", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				} else {
					notification("This user doesn't have an avatar!", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
			}
			...
			if (isset($_POST["delete_background"])) {
				...
				if ($Background !== NULL) {
					unlink($Background);
					notification("Background successfully deleted!", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				} else {
					notification("This user doesn't have a background!", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
			}
			...
			if (isset($_POST["delete_videos"])) {
				...
				if ($DB->RowNum > 0) {
					...
					notification("User videos successfully deleted!", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				} else {
					notification("This user doesn't have any videos!", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
			}
			...
			if ((isset($_POST["strike_user"]) || isset($_POST["ban_user"])) && $_POST["strike_reason"] != "none") {
					...
					preg_replace_callback("/http[s]?:\/\/(?:[(a-z)]+\.)?vidlii\.com\/.*?(?=[\&\?]v=([a-zA-Z0-9\_\-]{11}))/", function ($matches) {
						...
						for ($i = 1; $i < count($matches); $i += 2) {
							...
							if (!in_array($url, $VidsArray)) {
								...
								$check = $DB->execute(
									"SELECT title, description FROM videos WHERE url = :URL AND uploaded_by = :USER",
									true,
									[
										":URL" => $url,
										":USER" => $_GET["u"]
								...
								if ($DB->RowNum > 0) {
									...
									$DB->modify(
										"UPDATE videos SET status = '-3' WHERE url = :URL AND uploaded_by = :USER",
										[
											":URL" => $url,
											":USER" => $_GET["u"]
										]
									...
									$DB->modify(
										"UPDATE users SET videos = videos - 1 WHERE username = :USER",
										[
											":USER" => $_GET["u"]
										]
									);
									...
									$DB->modify(
										"DELETE FROM videos_flags WHERE url = :URL",
										[
											":URL" => $url
										]
									);
				...
				$DB->modify(
					"INSERT INTO strikes SET username = :USER, issued_by = :ISSUER, issued_on = :TIME, video_links = :LINKS, comment = :COMMENT",
					[
						":USER" => $_GET["u"],
				...
				if ($DB->RowNum == 0) {
					notification("Something went wrong, strike not inserted into table.", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}

				$DB->modify(
					"INSERT INTO private_messages SET to_user = :USER, from_user = :ISSUER, subject = :SUBJECT, message = :MESSAGE, date_sent = NOW()",
					[
						":USER" => $_GET["u"],
						":ISSUER" => $_USER->username,
						":SUBJECT" => $subject,
				...
				if ($DB->RowNum == 0) {
					notification("Something went wrong, PM not sent to user.", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
				...
				$DB->modify(
					"UPDATE users SET strikes = strikes + 1, ban_reasons = CONCAT(ban_reasons, :BAN) WHERE username = :USER LIMIT 1",
					[
						":USER" => $_GET["u"],
						":BAN" => $ban_reason
					]
				...
				if ($DB->RowNum == 0) {
					notification("Something went wrong, user strikes not updated.", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
				...
				if ($DB->RowNum == 0) {
					notification("Something went wrong, not able to check user strikes.", "/admin/users?u=" . $_GET["u"], "red");
					exit();
				}
				...
				if ($Strikes >= 3 || isset($_POST["ban_user"])) {
					...
					notification("Strike successfully sent.", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				}
			}
			...
			if (isset($_POST["ban_user"])) {
				...
				if ($Check == 0) {
					...
					notification($_GET["u"] . " has been banned!", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				} else {
					...
					notification($_GET["u"] . " has been unbanned!", "/admin/users?u=" . $_GET["u"], "green");
					exit();
				}
			}
			...
			if (isset($_POST["save_user"])) {
				...
				if ($Validation) {
					...
					if ($Validation["channel_type"] >= 0 && $Validation["channel_type"] <= 7) {
						$Channel_Type = $Validation["channel_type"];
					} else {
						$Channel_Type = 0;
					}
					if ($Validation["channel_version"] == 1 || $Validation["channel_version"] == 2 || $Validation["channel_version"] == 3) {
						$Channel_Version = $Validation["channel_version"];
					} else {
						$Channel_Version = 1;
					}
					if ($Validation["activated"] == 1 || $Validation["activated"] == 2) {
						$Activated = $Validation["activated"];
					} else {
						$Activated = 0;
					}
					if (array_key_exists($Validation["country"], $Countries)) {
					...
					if (get_age($Birthday) >= 13) {
						...
						if (strcasecmp($Validation["displayname"], $User["username"]) !== 0) {
							...
							if (!$Exists1 && !$Exists2) {
								...
								$DB->modify(
									"INSERT INTO users_oldnames SET displayname = :DNAME, username = :UNAME",
									[
								...
								$DB->modify(
									"UPDATE users SET displayname = :DNAME WHERE username = :UNAME LIMIT 1",
									[
										":DNAME" => $Display_Name,
										":UNAME" => $Edit_User->Info["username"]
									]
						...
						notification("User successfully updated!", "/admin/users?u=" . $_GET["u"], "green");
						exit();
					} else {
						notification("The user must be at least 13 years old!", "/admin/users?u=" . $_GET["u"], "red");
						exit();
					}
				}
			}
