...
if ($_USER->logged_in && ($_USER->Is_Admin || $_USER->Is_Mod) && isset($_SESSION["admin_panel"])) {
    ...
    if (!isset($_GET["v"])) {
        ...
        if ($DB->RowNum == 0) {
            notification("No videos could be found!", "/admin/videos");
            exit();
        }
        ...
        if ($DB->RowNum == 1) {
            ...
            if (isset($_POST["save_video"])) {
                ...
                if ($Validation) {
                    if ($Validation["month"] > 0 && $Validation["month"] <= 9) {
                        $Validation["month"] = "0" . $Validation["month"];
                    }

                    if ($Date == $Validation["year"] . "-" . $Validation["month"] . "-" . $Validation["day"]) {
                        $Upload_Date = $Video["uploaded_on"];
                    } else {
                        $Upload_Date = $Validation["year"] . "-" . $Validation["month"] . "-" . $Validation["day"];
                    }
                    if ($Validation["featured"] == 0 || $Validation["featured"] == 1) {
                    ...
                    $DB->modify(
                        "UPDATE videos SET title = :TITLE, description = :DESCRIPTION, tags = :TAGS, category = :CATEGORY, views = :VIEWS, displayviews = :DISPLAY_VIEWS, 1_star = :1_STAR, 2_star = :2_STAR, 3_star = :3_STAR, 4_star = :4_STAR, 5_star = :5_STAR, featured = :FEATURED WHERE url = :URL",
                    ...
                    if ($DB->RowNum == 1) {
                        if ($New_Display_Views != 0) {
                            ...
                            $DB->modify(
                                "INSERT INTO videos_views (vid, views, submit_date, source) VALUES (:VID, $Action2$New_Display_Views, NOW(), :SOURCE) ON DUPLICATE KEY UPDATE views = views $Action2 $New_Display_Views",
                                [
                                    ":VID" => $Video["url"],
                                    ":SOURCE" => "v"
                                ]
