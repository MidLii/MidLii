...
if ($Analytics_Page == "Default") {
    ...
    $Subscribers = $DB->execute(
        "SELECT count(users.username) as amount, users.country FROM subscriptions INNER JOIN users ON users.username = subscriptions.subscriber WHERE subscriptions.subscription = :USERNAME $Subscriber_Date GROUP BY users.country ORDER BY amount DESC LIMIT 5",
        false,
        [
    ...
    $Average_Age = $DB->execute(
        "SELECT AVG(TIMESTAMPDIFF(YEAR, users.birthday, CURDATE())) as average FROM subscriptions INNER JOIN users ON users.username = subscriptions.subscriber WHERE subscriptions.subscription = :USERNAME AND (TIMESTAMPDIFF(YEAR, users.birthday, CURDATE())) > 13 AND (TIMESTAMPDIFF(YEAR, users.birthday, CURDATE())) < 70",
        true,
        [
            ":USERNAME" => $_USER->username
        ]
    ...
    $Average_Rating = $DB->execute(
        "SELECT (1_star * 1 + 2_star * 2 + 3_star * 3 + 4_star * 4 + 5_star * 5) / (1_star + 2_star + 3_star + 4_star + 5_star) as average FROM videos WHERE uploaded_by = :USERNAME",
        false,
        [
            ":USERNAME" => $_USER->username
        ]
    ...
    $Views_Per_Day = $DB->execute(
        "SELECT SUM(videos_views.views) as average FROM videos_views INNER JOIN videos ON videos_views.vid = videos.url WHERE videos.uploaded_by = :USERNAME GROUP BY DATE(videos_views.submit_date)",
        false,
        [
            ":USERNAME" => $_USER->username
        ]
    ...
    $Watchtime_Per_Day = $DB->execute(
        "SELECT (SUM(videos_watched.watchtime) / 60) as amount, videos_watched.submit_date FROM videos_watched INNER JOIN videos ON videos_watched.vid = videos.url WHERE videos.uploaded_by = :USERNAME $Watch_Date GROUP BY DATE(videos_watched.submit_date)",
        false,
        [
            ":USERNAME" => $_USER->username
        ]
    ...
    if (!isset($_GET["type"]) || $_GET["type"] == 0) {
        ...
        $Subscribers_Growth = $DB->execute(
            "SELECT count(subscriptions.subscriber) as amount, subscriptions.submit_date FROM subscriptions INNER JOIN users ON users.username = subscriptions.subscriber WHERE subscriptions.subscription = :USERNAME $Subscriber_Date $Country AND subscriptions.submit_date <> '0000-00-00' GROUP BY DATE(subscriptions.submit_date)",
            false,
            [
                ":USERNAME" => $_USER->username
            ]
        ...
        $Subscribers = $DB->execute(
            "SELECT count(users.username) as amount, users.country FROM subscriptions INNER JOIN users ON users.username = subscriptions.subscriber WHERE subscriptions.subscription = :USERNAME $Subscriber_Date $Country GROUP BY users.country ORDER BY amount DESC LIMIT 100",
            false,
            [
                ":USERNAME" => $_USER->username
            ]
        ...
        $Subscribers = $DB->execute(
            "SELECT count(subscriptions.subscriber) as amount, subscriptions.source FROM subscriptions WHERE subscriptions.subscription = :USERNAME $Subscriber_Date GROUP BY subscriptions.source ORDER BY amount DESC LIMIT 30",
            false,
            [
                ":USERNAME" => $_USER->username
            ]
    ...
    if (!isset($_GET["video"])) {
        ...
        $Videos_Growth = $DB->execute(
            "SELECT sum(videos_views.views) as amount, videos_views.vid, videos_views.submit_date FROM videos_views INNER JOIN videos ON videos_views.vid = videos.url WHERE videos.uploaded_by = :USERNAME $Video_Date GROUP BY DATE(videos_views.submit_date), videos.uploaded_by",
            false,
            [
                ":USERNAME" => $_USER->username
            ]
        ...
        $Videos_Growth = $DB->execute(
            "SELECT sum(videos_views.views) as amount, videos_views.vid, videos_views.submit_date FROM videos_views INNER JOIN videos ON videos_views.vid = videos.url WHERE (videos.uploaded_by = :USERNAME OR '$_USER->Is_Admin' = '1' OR '$_USER->Is_Mod' = '1') AND videos.url = :URL $Video_Date GROUP BY DATE(videos_views.submit_date)",
            false,
            [
                ":URL" => $_GET["video"],
                ":USERNAME" => $_USER->username
        ...
        $Selected_Video = $DB->execute(
            "SELECT sum(videos_views.views) as amount, videos_views.source FROM videos_views RIGHT JOIN videos ON videos_views.vid = videos.url WHERE (videos.uploaded_by = :USERNAME OR '$_USER->Is_Admin' = '1' OR '$_USER->Is_Mod' = '1') AND videos.url = :URL $Video_Date GROUP BY videos_views.source ORDER BY amount DESC",
            false,
            [
                ":URL" => $_GET["video"],
                ":USERNAME" => $_USER->username
