...
if (isset($_POST["update_holiday"])) {
    ...
    $DB->modify("UPDATE users SET snow = :SNOW, mondo = :MONDO WHERE username = :USERNAME", [
        ":SNOW" => $Snow,
        ":MONDO" => $Mondo,
        ":USERNAME" => $_USER->username,

    ]);
    notification("Holiday settings updated!", "/channel_setup", "green");
    exit();
}
...
if (isset($_POST["update_channel"])) {
    ...
    if ($Validation) {
        ...
        if ($Type >= 0 && $Type <= 7) {
            $DB->modify(
                "UPDATE users SET channel_type = '$Type', channel_title = :TITLE, channel_description = :DESCRIPTION, channel_tags = :TAGS WHERE username = :USERNAME",
                [
                    ":TITLE" => $Channel_Title,
            ...
            notification("Account successfully updated!", "/channel_setup", "green");
            exit();
        }
    }
}
...
if (isset($_POST["update_privacy"])) {
    ...
    if ($can_friend != 1)
        $can_friend = 0;
    if ($ch_privacy != 1 && $ch_privacy != 2)
        $ch_privacy = 0;

    $DB->modify(
        "UPDATE users SET privacy = :PRIVACY, can_friend= :CAN_FRIEND WHERE username = :USERNAME",
        [
            ":PRIVACY" => $ch_privacy,
