...
if (!$ajaxCall) {
	if ($URL !== false) {
		...
		if ($Video->Info["uploaded_by"] !== $_USER->username) {
			notification("You don't have the permissions to edit this video!", "/my_videos", "red");
		}
	} else {
		notification("This video doesn't exist!", "/my_videos", "red");
		exit();
	}
} else {
	if ($URL === false) {
...
if (isset($_POST["update_info"]) || $ajaxCall) {
	...
	if ($Validation || $ajaxCall) {
			...
			if (!($_POST["c_thmp"] == 1 && $Has_Custom_Thumbnail)) {
				for ($i = 0; $i < 2; $i++) {
					...
					if (!$Uploader->processed) {
						...
						if (!$ajaxCall)
							notification($err, "/edit_video?v=$URL", "red");
						else
							die("0|$err");
					} else {
						@unlink("usfi/$DIR/$URL.temp.jpg");
						if ($ajaxCall)
							die("1|/usfi/$DIR/$URL.jpg");
					}
				}
			}
		...
		if (!$ajaxCall || !check_captcha_sp(4)) {
			...
			if ($_POST["comments"] == 1) {
				$Comments = 1;
			} elseif ($_POST["comments"] == 2) {
				$Comments = 2;
			} else {
				$Comments = 0;
			}

			if ($_POST["responses"] == 1) {
				$Responses = 1;
			} elseif ($_POST["responses"] == 2) {
				$Responses = 2;
			} else {
				$Responses = 0;
			}
			...
			$DB->modify(
				"UPDATE videos SET s_related = $Related, s_comments = $Comments, s_responses = $Responses, s_ratings = $Ratings, privacy = $Privacy, most_popular = $Most_Popular, title = :TITLE, description = :DESCRIPTION, tags = :TAGS, category = :CATEGORY WHERE url = :URL AND uploaded_by = :USERNAME",
			...
			notification("Video successfully updated", "/edit_video?v=$URL", "green");
			exit();
		}
	}
}
