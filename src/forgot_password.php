...
if ($_USER->logged_in && !isset($_GET["reject"])) {
    redirect("/");
    exit();
}
...
if (!isset($_GET["code"]) && !isset($_GET["reject"])) {
    if (isset($_POST["submit_forgot"])) {
        if (isset($_POST["vl_username"]) && ctype_alnum($_POST["vl_username"]) && check_captcha()) {
            ...
            if ($DB->RowNum == 1) {
                ...
                $Insert = $DB->modify(
                    "INSERT INTO forgot_password VALUES(:USERNAME, :CODE)",
                    [
                        ":USERNAME" => $Username,
                        ":CODE" => $Code
                    ]
    ...
    if ($DB->RowNum == 1) {
        if (isset($_POST["submit_password"])) {
            ...
            if (isset($_POST["vl_password"], $_POST["vl_password2"]) && strlen($_POST["vl_password"]) <= 128 && strlen($_POST["vl_password"]) > 4 && $_POST["vl_password"] === $_POST["vl_password2"]) {
                ...
                if ($DB->RowNum == 1) {
                    $DB->modify(
                        "DELETE FROM forgot_password WHERE username = :USERNAME AND code = :CODE",
                        [
                            ":USERNAME" => $Username,
                            ":CODE" => $Check["code"]
                        ]
                    );
                    notification("You can now log in with your new password!", "/login", "green");
                    exit();
                }
            } else {
                notification("The Passwords must match!", "/forgot_password?code=" . $_GET["code"], "red");
                exit();
            }
        }
        ...
        notification("This is an invalid code!", "/login", "red");
        exit();
    }
    ...
    if ($DB->RowNum == 1) {
        notification("Thank you!", "/", "green");
        exit();
    } else {
        notification("The password has already been recovered!", "/", "red");
        exit();
    }
