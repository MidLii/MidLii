...
// if (!$_USER->Is_Activated)    { redirect("/terms"); exit();        }
...
$Status = 2;
$Autoplay = false;
$URL = is_array($Most_Viewed) ? $Most_Viewed["url"] : "";
$FILENAME = is_array($Most_Viewed) ? $Most_Viewed["file"] : "";
$ISHD = is_array($Most_Viewed) && $Most_Viewed["hd"] == 1 ? true : false;
$Length = is_array($Most_Viewed) ? $Most_Viewed["seconds"] : 0;
...
if (!isset($_COOKIE["s"]) || !$_USER->logged_in) {
    ...
    if ($_USER->logged_in) {
        if (isset($_POST["save_modules"])) {
            ...
            notification("Homepage successfully updated!", "/", "green");
            exit();
        }
    ...
    if ($Subscriptions_Amount > 0 || $Friends_Amount > 0) {
        ...
        if (!isset($_COOKIE["st"]) && $Friends_Amount > 0) {
            if (!isset($_GET["t"]) || $_GET["t"] == 1) {
                $SELECT = "SELECT 'bulletin' as type_name, by_user as id, content, date as date, '' as title, 'a' as video_by, 'a' as video_desc FROM bulletins WHERE (by_user IN ($SQL_USERS) OR by_user = '$_USER->username') AND bulletins.content NOT LIKE '%vanillo%' ";
            }
            if (!isset($_GET["t"]) || $_GET["t"] == 2) {
                if (!isset($_GET["t"])) {
                    $SELECT .= "UNION ALL";
                }
                $SELECT .= " SELECT 'comment' as type_name, videos.url as id, video_comments.comment as content, video_comments.date_sent as date, videos.title as title, video_comments.by_user as video_by, videos.description as video_desc FROM video_comments INNER JOIN videos ON video_comments.url = videos.url WHERE by_user IN ($SQL_USERS) ";
            }
            if (!isset($_GET["t"]) || $_GET["t"] == 3) {
                if (!isset($_GET["t"])) {
                    $SELECT .= "UNION ALL";
                }
                $SELECT .= " SELECT 'favorite' as type_name, videos.url as id, videos.description as content, video_favorites.date as date, videos.title as title, video_favorites.favorite_by as video_by, 'a' as video_desc FROM video_favorites INNER JOIN videos ON video_favorites.url = videos.url WHERE favorite_by IN ($SQL_USERS) ";
            }
            if (!isset($_GET["t"])) {
                $SELECT .= "UNION ALL SELECT 'friend' as type_name, friend_1, friend_2, sent_on, '' as title, 'a' as video_by, 'a' as video_desc FROM friends as date WHERE (friend_1 IN ($SQL_USERS) OR friend_2 IN ($SQL_USERS)) AND status = 2 ";
            }
            if (!isset($_GET["t"])) {
                $SELECT .= "UNION ALL SELECT 'sub' as type_name, videos.url, '' as comment, videos.uploaded_on as date, videos.title as title, videos.uploaded_by as video_by, videos.description as video_desc FROM videos WHERE videos.uploaded_by IN($SQL_SUBS) AND videos.status = 2 ";
            }
        } else {
