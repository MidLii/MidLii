...
if ($uchange >= 15552000) {
    ...
    if ($uchange < 120)
        $time = "1 minute";
    else if ($uchange < 3600)
        $time = (int) ($uchange / 60) + 1 . " minutes";
    else if ($uchange < 86400)
        $time = (int) ($uchange / 3600) + 1 . " hours";
    else if ($uchange < 2592000)
        $time = (int) ($uchange / 86400) + 1 . " days";
    else
        $time = (int) ($uchange / 2592000) + 1 . " months";
...
if (isset($_POST["save_email"]) && $_SESSION["token"] == $_POST["cst"]) {
    ...
    if ($Validation && $_USER->Info["email"] !== $_POST["email"]) {
        ...
        if (password_verify($Password, $_USER->Info["password"]) && $DB->RowNum == 0) {
            ...
            notification("Email was successfully updated!", "/manage_account", "green");
            exit();
        } else {
            if (!password_verify($Password, $_USER->Info["password"])) {
                notification("The password you entered is incorrect!", "/manage_account", "red");
                exit();
            } elseif ($_DATABASE->exists($Email, "email", "users")) {
                notification("This email is already in use!", "/manage_account", "red");
                exit();
            }
        }
    } else {
        if (mb_strlen($_POST["password"]) < 4) {
            notification("The password must be over 4 letters long!", "/manage_account", "red");
            exit();
        } elseif ($_USER->Info["email"] == $_POST["email"]) {
            notification("You're using this email at the moment!", "/manage_account", "red");
            exit();
        }
    }
}
...
if (isset($_POST["save_password"]) && $_SESSION["token"] == $_POST["cst"]) {
    ...
    if ($Validation) {
        if (password_verify($Validation["current_password"], $_USER->Info["password"])) {
            ...
            notification("Password was successfully changed!", "/manage_account", "green");
            exit();
        } else {
            notification("The password you entered is incorrect!", "/manage_account", "red");
            exit();
        ...
        if ($_POST["new_password"] !== $_POST["new_password2"]) {
            notification("The two passwords you entered don't match!", "/manage_account", "red");
            exit();
        } elseif (mb_strlen($_POST["current_password"]) < 4 || mb_strlen($_POST["new_password"]) < 4) {
            notification("Passwords must be over 4 letters long!", "/manage_account", "red");
            exit();
        }
    }
}
    ...
    if ($Validation) {
        ...
        if ($New_Name != $_USER->displayname && $New_Name !== "0" && $New_Name !== 0) {
            ...
            if (!$ExistsError) {
                ...
                if (!$UpdateError) {
                    $DB->modify(
                        "UPDATE users SET displayname = :DNAME, last_username_update = :LAST WHERE username = :UNAME LIMIT 1",
                        [
                            ":DNAME" => $New_Name,
                            ":UNAME" => $_USER->username,
                            ":LAST" => time()
