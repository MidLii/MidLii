...
if (isset($_POST["create_playlist"])) {
    ...
    if ($Validation) {
        ...
        $Create = $DB->modify(
            "INSERT INTO playlists (purl,title,created_by,thumbnail,created_on) VALUES (:PURL,:TITLE,:USERNAME,:THUMBNAIL,NOW())",
            [
                ":PURL" => $PURL,
...
if (!isset($_GET["pl"])) {
    ...
    $Playlist = $DB->execute(
        "SELECT purl, title, created_on, thumbnail FROM playlists WHERE purl = :URL AND created_by = :USERNAME",
        true,
        [
            ":URL" => $_GET["pl"],
            ":USERNAME" => $_USER->username
    ...
    if ($DB->RowNum == 1) {
        ...
        if (isset($_POST["update_playlist"])) {
            ...
            if ($Validation) {
                ...
                $DB->modify(
                    "UPDATE playlists SET title = :TITLE WHERE purl = :PURL AND created_by = :USERNAME",
                    [
                        ":TITLE" => $Name,
                        ":PURL" => $Playlist["purl"],
                        ":USERNAME" => $_USER->username
                ...
                redirect("my_playlists?pl=" . $Playlist["purl"]);
                exit();
            }
        }
            ...
            if ($Validation) {
                if (strpos($Validation["video"], "watch?v=") !== false) {
                    ...
                    if ($URL !== false) {
                        ...
                        $DB->modify(
                            "INSERT IGNORE INTO playlists_videos (url,purl,position) VALUES (:URL,:PURL,:POSITION)",
                            [
                                ":URL" => $URL,
                                ":PURL" => $PURL,
                        ...
                        if ($DB->RowNum == 1) {
                            $DB->modify(
                                "UPDATE playlists SET thumbnail = :URL WHERE purl = :PURL AND created_by = :USERNAME",
                                [
                                    ":URL" => $URL,
                                    ":PURL" => $PURL,
                                    ":USERNAME" => $_USER->username
                            ...
                            notification("You successfully added the video to this playlist!", "/my_playlists?pl=" . $_GET["pl"], "green");
                            exit();
                        } else {
                            notification("This video is already inside this playlist!", "/my_playlists?pl=" . $_GET["pl"], "red");
                            exit();
                        }
                    }
                }
