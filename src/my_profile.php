...
if (isset($_POST["update_info"])) {
    ...
    if ($Validation) {
        ...
        if (get_age($Birthday) >= 13) {
            ...
            $DB->modify(
                "UPDATE users SET about = :ABOUT, website = :WEBSITE, birthday = :BIRTHDAY, i_occupation = :OCCUPATION, i_schools = :SCHOOLS, i_interests = :INTERESTS, i_movies = :MOVIES, i_music = :MUSIC, i_books = :BOOKS, a_age = :AGE, a_country = :COUNTRY, a_last = :SIGNIN WHERE username = :USERNAME",
            ...
            notification("Profile successfully updated!", "/my_profile", "green");
            exit();
        } else {
            notification("You must be at least 13 years old to use VidLii!", "/my_profile", "red");
            exit();
        }
    }
}
