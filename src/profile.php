...
if (strtoupper($_GET["user"]) == "WUFF") {
    ...
    if ($profiles_enabled == 0) {
        notification("Channels have been temporarily disabled!", "/");
        exit();
    }
            ...
            if ($Profile["channel_version"] == 1 && $Profile["banned"] == 0) {
                if ($_USER->logged_in && $_USER->username === $Profile["username"]) {
                    ...
                    if (isset($_POST["change_ra"])) {
                        if (isset($_POST["ra_comments"])) {
                            $RA_COMMENTS = 1;
                        } else {
                            $RA_COMMENTS = 0;
                        }
                        if (isset($_POST["ra_favorites"])) {
                            $RA_FAVORITES = 1;
                        } else {
                            $RA_FAVORITES = 0;
                        }
                        if (isset($_POST["ra_friends"])) {
                            $RA_FRIENDS = 1;
                        } else {
                            $RA_FRIENDS = 0;
                        }
                        ...
                        $DB->modify(
                            "UPDATE users SET ra_comments = :COMMENTS, ra_favorites = :FAVORITES, ra_friends = :FRIENDS WHERE username = :USERNAME",
                            [
                    ...
                    //CHECK IF FRIENDS
                    $Friend_Status = $DB->execute(
                        "SELECT status, by_user FROM friends WHERE (friend_1 = :USERNAME AND friend_2 = :OWNER) OR (friend_1 = :OWNER AND friend_2 = :USERNAME)",
                        true,
                        [
                            ":USERNAME" => $_USER->username,
                ...
                if ($Profile["privacy"] == 2 && $Is_Friends == false && $_USER->username != $Profile["username"]) {
                    notification("You're not allowed to see this page!", "/", "red");
                    exit();
                }
                ...
                if ($Profile["subscriptions"] > 0 and $Profile["c_subscription"] && (empty($_GET["page"]) || is_numeric($_GET["page"]) || $_GET["page"] == "subscriptions")) {
                    ...
                    if ($_USER->logged_in && $Profile["subscriptions"] > 0 && !$Is_OWNER) {
                        $DB->execute(
                            "SELECT subscriber FROM subscriptions WHERE subscriber = :USERNAME AND subscription = :YOU",
                            false,
                            [
                ...
                if ($Profile["videos"] > 0 and $Profile["c_videos"] && (empty($_GET["page"]) || is_numeric($_GET["page"]) || $_GET["page"] == "videos")) {
                    ...
                    if (isset($_POST["search_input"]) && !empty($_POST["search"]) && mb_strlen($_POST["search"]) >= 3 && mb_strlen($_POST["search"]) <= 64) {
                        ...
                        if ($Videos::$Videos) {
                            ...
                            redirect("/user/" . $Profile["displayname"] . "/videos");
                            exit();
                        }
                    }
                }
                ...
                if ($Profile["favorites"] > 0 and $Profile["c_favorites"] && (empty($_GET["page"]) || is_numeric($_GET["page"]) || $_GET["page"] == "favorites")) {
                    ...
                    if (isset($_POST["search_input"]) && !empty($_POST["search"]) && mb_strlen($_POST["search"]) >= 3 && mb_strlen($_POST["search"]) <= 64) {
                        ...
                        if ($Favorites::$Videos) {
                            ...
                            redirect("/user/" . $Profile["displayname"] . "/favorites");
                            exit();
                        }
                    }
                }
                ...
                if ($_USER->logged_in && isset($_POST["save_custom"])) {
                    ...
                    if (mb_strlen($BBCODE) <= 1024) {
                        $DB->modify(
                            "UPDATE users SET custom = :CUSTOM WHERE username = :USERNAME",
                            [
                                ":CUSTOM" => $BBCODE,
                                ":USERNAME" => $_USER->username
                            ]
                        );
                        redirect("/user/$_USER->username");
                        exit();
                    }
                }
                ...
                if ($Profile["c_featured_channels"] == "1" && !empty($Profile["featured_channels"])) {
                    ...
                    if (strpos($Featured_Channels_Array, ",") !== false) {
                        $ORDER = "ORDER BY FIELD($Featured_Channels_Array)";
                    } else {
                        $ORDER = "";
                    }
                ...
                if ($Profile["c_recent"] == "1") {
                    ...
                    if ($Profile["ra_comments"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'comment' as type_name, videos.url, video_comments.comment, video_comments.date_sent as date, videos.title as title FROM video_comments INNER JOIN videos ON video_comments.url = videos.url WHERE by_user = :OWNER AND videos.status = 2 AND videos.privacy = 0 AND videos.banned_uploader = 0 ";
                    }
                    if ($Profile["ra_favorites"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'favorite' as type_name, videos.url, videos.description as comment, video_favorites.date as date, videos.title as title FROM video_favorites INNER JOIN videos ON video_favorites.url = videos.url WHERE favorite_by = :OWNER AND videos.status = 2 AND videos.privacy = 0 AND videos.banned_uploader = 0 ";
                    }
                    if ($Profile["ra_friends"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'friend' as type_name, friend_1, friend_2, sent_on, '' as title FROM friends as date WHERE (friend_1 = :OWNER OR friend_2 = :OWNER) AND status = 1";
                    }
                ...
                if ($_USER->logged_in && $_USER->username === $Profile["username"]) {
                    ...
                    if (isset($_POST["save_settings"])) {
                        ...
                        if ($Validation) {
                            ...
                            if ($Type >= 0 && $Type <= 7) {
                                $DB->modify(
                                    "UPDATE users SET channel_type = '$Type', channel_title = :TITLE, channel_tags = :TAGS WHERE username = :USERNAME",
                                    [
                                        ":TITLE" => $Channel_Title,
                                        ":TAGS" => $Channel_Tags,
                                        ":USERNAME" => $_USER->username
                                ...
                                redirect("/user/" . $_USER->displayname);
                                exit();
                            }
                        }
                    }
                ...
                if ($Profile["videos"] > 0 && $Profile["c_videos"] == 1) {
                    if ((($Profile["favorites"] == 0 || !$Profile["c_favorites"]) && (!$Profile["c_playlists"] || $Playlist_Amount == 0)) || (!$Profile["c_all"])) {
                        $LIMIT = 16;
                    } else {
                        $LIMIT = 3;
                    }
                    ...
                    if ($Videos::$Videos) {
                        ...
                        if ($Videos::$Amount < $Profile["videos"]) {
                            $Show_More = '<center><button id="show_more" onclick="show_more(\'videos\',\'1\')">Show More</button></center>';
                        } else {
                            $Show_More = "";
                        }
                        $Videos = $Videos->fixed();
                ...
                if ($Profile["favorites"] > 0 && $Profile["c_favorites"] == 1) {
                    if ((($Profile["videos"] == 0 || !$Profile["c_videos"]) && (!$Profile["c_playlists"])) || (!$Profile["c_all"])) {
                        $LIMIT = 16;
                    } else {
                        $LIMIT = 3;
                    }
                    ...
                    if ($Favorites::$Videos) {
                        ...
                        if (!isset($Show_More) && $Favorites::$Amount < $Profile["favorites"]) {
                            $Show_More = '<center><button id="show_more" onclick="show_more(\'favorites\',\'1\')">Show More</button></center>';
                        } elseif (!isset($Show_More)) {
                            $Show_More = "";
                        }
                        $Favorites = $Favorites->fixed();
                    ...
                    if ($Shows_Playlists == false && $Shows_Videos == false) {
                        $Showcase = false;
                    }
                ...
                if (isset($_POST["save_modules"])) {
                    ...
                    if (isset($_POST["featured_channels"])) {
                        $Featured_Channels = 1;
                    } else {
                        $Featured_Channels = 0;
                    }
                    ...
                    $Update = $DB->modify(
                        "UPDATE users SET c_custom = :CUSTOM, c_comments = :COMMENTS, c_friend = :FRIENDS, c_featured_channels = :FEATURED_CHANNELS, c_recent = :RECENT, c_subscriber = :SUBSCRIBER, c_subscription = :SUBSCRIPTIONS WHERE username = :USERNAME",
                        [
                            ":CUSTOM" => $Custom,
                            ":COMMENTS" => $c_comments,
                            ":FRIENDS" => $c_friends,
                ...
                if (isset($_POST["save_players"])) {
                    ...
                    if (isset($_POST["show_playlists"])) {
                        $Show_playlists = 1;
                    } else {
                        $Show_playlists = 0;
                    }

                    if ($_POST["pl_layout"] == 0) {
                        $Layout = 0;
                    } else {
                        $Layout = 1;
                    }
                    ...
                    $DB->modify(
                        "UPDATE users SET featured_n_url = :N_URL, featured_s_url = :S_URL, c_featured = :FT,  c_playlists = :C_PLAYLISTS, c_all = :C_ALL, c_videos = :C_VIDEOS, c_favorites = :C_FAVORITES, default_view = :LAYOUT WHERE username = :USERNAME",
                        [
                            ":FT" => $FT,
                            ":N_URL" => $N_URL,
                            ":S_URL" => $S_URL,
                ...
                if (isset($_POST["save_customization"])) {
                    ...
                    if ($Validation) {
                        ...
                        if (isset($Validation["n_trans"]) && $Validation["n_trans"] >= 0 && $Validation["n_trans"] <= 100) {
                            $Normal_Trans = (int) $Validation["n_trans"];
                        } else {
                            $Normal_Trans = 0;
                        }
                        if (isset($Validation["h_trans"]) && $Validation["h_trans"] >= 0 && $Validation["h_trans"] <= 100) {
                            $Highlight_Trans = (int) $Validation["h_trans"];
                        } else {
                            $Highlight_Trans = 0;
                        }
                        if (isset($Validation["ch_fnt"]) && $Validation["ch_fnt"] >= 0 && $Validation["ch_fnt"] <= 6) {
                            $Channel_Font = (int) $Validation["ch_fnt"];
                        } else {
                            $Channel_Font = 0;
                        }
                        ...
                        if (isset($Validation["bg_fixed"])) {
                            $bg_fixed = 1;
                        } else {
                            $bg_fixed = 0;
                        }
                        if (isset($Validation["bg_stretch"])) {
                            $bg_stretch = 1;
                        } else {
                            $bg_stretch = 0;
                        }
                        if (isset($Validation["chn_radius"]) && $Validation["chn_radius"] >= 0 && $Validation["chn_radius"] <= 9) {
                            $Channel_Radius = (int) $Validation["chn_radius"];
                        } else {
                            $Channel_Radius = 5;
                        }
                        if (isset($Validation["avt_radius"]) && $Validation["avt_radius"] >= 0 && $Validation["avt_radius"] <= 9) {
                            $Avatar_Radius = (int) $Validation["avt_radius"];
                        } else {
                            $Avatar_Radius = 4;
                        }


                        if ($Validation["theme"] >= 0 && $Validation["theme"] <= 9) {
                            $Theme = (int) $Validation["theme"];
                        } else {
                            $Theme = 0;
                        }

                        $DB->modify(
                            "UPDATE users SET avt_radius = :AVT_RADIUS, chn_radius = :CHN_RADIUS, font = :FONT, theme = :THEME, n_trans = :N_TRANS, h_trans = :H_TRANS, bg_repeat = :BG_REPEAT, bg_position = :BG_POSITION, bg_fixed = :BG_FIXED, bg_stretch = :BG_STRETCH, bg = :BG_COLOR, h_head = :WRP_COLOR, h_in_fnt = :WRP_TXT_COLOR, h_head_fnt = :WRP_LINK_COLOR, n_in = :NM_IN_COLOR, n_head_fnt = :TITLE_TXT_COLOR, links = :IN_LINK_COLOR, n_in_fnt = :IN_TXT_COLOR WHERE username = :USERNAME",
                            [
                ...
                if ($_GET["page"] === "playlist") {
                    ...
                    if (!empty($Main_Playlist) && $Main_Playlist != 1) {
                        $Check_Playlist = $DB->execute(
                            "SELECT purl, title FROM playlists WHERE created_by = :OWNER AND purl = :PURL",
                            true,
                            [
                                ":OWNER" => $Profile["username"],
                                ":PURL" => $Main_Playlist
                ...
                if ($Profile["c_recent"] == "1") {
                    ...
                    if ($Profile["ra_comments"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'comment' as type_name, videos.url, videos.description, video_comments.date_sent as date, videos.title as title, videos.length as length, videos.displayviews as views FROM video_comments INNER JOIN videos ON video_comments.url = videos.url WHERE by_user = :OWNER ";
                    }
                    if ($Profile["ra_favorites"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'favorite' as type_name, videos.url, videos.description as comment, video_favorites.date as date, videos.title as title, videos.length as length, videos.displayviews as views FROM video_favorites INNER JOIN videos ON video_favorites.url = videos.url WHERE favorite_by = :OWNER ";
                    }
                    if ($Profile["ra_friends"] == 1) {
                        $SELECT .= "UNION ALL SELECT 'friend' as type_name, friend_1, friend_2, sent_on, '' as title, '' as length, '' as views FROM friends as date WHERE (friend_1 = :OWNER OR friend_2 = :OWNER) AND status = 1 ";
                    }
                ...
                if (isset($_POST["cosmic_save_about"]) && $_USER->logged_in) {
                    ...
                    if ($Validation) {
                        ...
                        if (isset($_POST["cosmic_last"])) {
                            $Last_Login = 1;
                        } else {
                            $Last_Login = 0;
                        }
                        if (isset($_POST["cosmic_show_country"])) {
                            $Show_Country = 1;
                        } else {
                            $Show_Country = 0;
                        }
                        if (array_key_exists($Validation["cosmic_country"], $Countries)) {
                            $Country = $Validation["cosmic_country"];
                        } else {
                            $Country = $Profile["country"];
                        }

                        $DB->modify(
                            "UPDATE users SET website = :WEBSITE, country = :COUNTRY, channel_description = :DESCRIPTION, a_country = :SHOW_COUNTRY, a_last = :SHOW_LAST WHERE username = :USERNAME",
                            [
                ...
                if ($_USER->logged_in && isset($_POST["save_featured"])) {
                    ...
                    $DB->modify(
                        "UPDATE users SET featured_n_url = :N_URL, featured_s_url = :S_URL WHERE username = :USERNAME",
                        [
                            ":N_URL" => $N_URL,
                            ":S_URL" => $S_URL,
                            ":USERNAME" => $_USER->username
                ...
                if (isset($_POST["save_recent"]) && $_USER->logged_in) {
                    if (isset($_POST["recent_comments"])) {
                        $Recent_Comments = 1;
                    } else {
                        $Recent_Comments = 0;
                    }
                    if (isset($_POST["recent_favorites"])) {
                        $Recent_Favorites = 1;
                    } else {
                        $Recent_Favorites = 0;
                    }
                    if (isset($_POST["recent_friends"])) {
                        $Recent_Friends = 1;
                    } else {
                        $Recent_Friends = 0;
                    }
                    ...
                    redirect("/user/$_USER->displayname/feed");
                    exit();
                }
                ...
                if (isset($_POST["save_comments"]) && $_USER->logged_in) {
                    ...
                    redirect("/user/$_USER->displayname/comments");
                    exit();
                }
                ...
                if (isset($_POST["save_channel_branding"]) && $_USER->logged_in) {
                    ...
                    if ($Validation) {
                        ...
                        if (isset($_POST["c_videos"])) {
                            $Videos = 1;
                        } else {
                            $Videos = 0;
                        }
                        if (isset($_POST["c_favorites"])) {
                            $Favorites = 1;
                        } else {
                            $Favorites = 0;
                        }
                        if (isset($_POST["c_playlists"])) {
                            $Playlists = 1;
                        } else {
                            $Playlists = 0;
                        }
                        ...
                        $DB->modify(
                            "UPDATE users SET bg = :BG_COLOR, channel_title = :TITLE, channel_tags = :TAGS, c_recent = $Recent, c_comments = $Comments, c_videos = $Videos, c_favorites = $Favorites, c_playlists = $Playlists, bg_position = :POSITION, bg_repeat = :REPEAT, bg_fixed = :FIXED WHERE username = '$_USER->username'",
                            [
                ...
                if (isset($_POST["save_featured_channels"]) && $_USER->logged_in) {
                    ...
                    if (count($Channels) <= 8) {
                        ...
                        if ($Error == false) {
                            ...
                            $Update = $DB->modify(
                                "UPDATE users SET featured_channels = :CHANNELS, featured_title = :TITLE WHERE username = '$_USER->username'",
                                [
                                    ":CHANNELS" => $Channels,
                ...
                if ($_GET["page"] == "comments" && $Profile["c_comments"] && $Profile["channel_comments"] > 0) {
                    ...
                    if ($DB->RowNum < $Profile["channel_comments"]) {
                        $Show_More = true;
                    } else {
                        $Show_More = false;
                    }
                ...
                {
                    if (strpos($Avatar, "u=") !== false) {
                        $Avatar = str_replace("u=", "", $Avatar);
                        $Folder = "avt";
                    } else {
                        $Upload = false;
                        $Folder = "thmp";
                ...
                function thumbnail_picture($URL, $LENGTH, $Width, $Height, $Title = NULL)
                {
                    if (!empty($LENGTH) || $LENGTH == "0") {
                        $Length = seconds_to_time((int) $LENGTH);
                    } else {
                        $Length = $LENGTH;
                    }
                    if (file_exists("usfi/thmp/$URL.jpg")) {
                        $Thumbnail = "/usfi/thmp/$URL.jpg";
                    } else {
                        $Thumbnail = "/img/no_th.jpg";
                    }
