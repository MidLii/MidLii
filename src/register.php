...
if ($DB->execute("SELECT value FROM settings WHERE name = 'signup'", true)["value"] == 0) {
    notification("Registrations have been temporarily disabled!.", "/", "red");
    exit();
}
...
//        $Validation["email"] = str_replace(".","",$Email_Parts[0])."@".$Email_Parts[1];
        ...
        if ($Ban_Amount > 22 || isset($_COOKIE["colorsvl"])) {
            ...
            notification("You have too many accounts.", "/register", "red");
            exit();

        }

        //STOPFORUMSPAM
        $IP_VAL = file_get_contents("http://api.stopforumspam.org/api?ip=$IP");
        if (strpos($IP_VAL, "<appears>yes</appears>") !== false) {
            setcookie("f4g", $Username, time() + (96400 * 1200), "/");
        ...
        if (get_age($Birthday) >= 13 && array_key_exists($Country, $Countries)) {
            ...
            if (!$User_Exists && !$User_Exists2 && !$Email_Exists) {
                ...
                //CREATE USER
                $test = $DB->modify(
                    "INSERT INTO users (username,displayname,email,password,reg_date,last_login,birthday,1st_latest_ip,country) VALUES (:USERNAME,:DISPLAYNAME,:EMAIL,:PASSWORD,NOW(),NOW(),:BIRTHDAY,:IP,:COUNTRY)",
                    [
                        ":USERNAME" => $Username,
                ...
                $Insert = $DB->modify(
                    "INSERT INTO activations VALUES (:USERNAME,:SECRET)",
                    [
                        ":USERNAME" => $Username,
