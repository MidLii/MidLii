...
if (isset($_POST["upload_theme"]) && $_USER->logged_in) {
	...
	if ($Validation) {
		...
		if (isset($Validation["chrome"])) {
			$Chrome = 1;
		} else {
			$Chrome = 0;
		}
		if (isset($Validation["firefox"])) {
			$Firefox = 1;
		} else {
			$Firefox = 0;
		}
		if (isset($Validation["edge"])) {
			$Edge = 1;
		} else {
			$Edge = 0;
		}
		if (isset($Validation["internet"])) {
			$Internet = 1;
		} else {
			$Internet = 0;
		}
		if (isset($Validation["opera"])) {
			$Opera = 1;
		} else {
			$Opera = 0;
		}
		...
		if (!empty($_FILES["main_picture"]["name"]) && !empty($_FILES["secondary_picture"]["name"]) && !empty($_FILES["css_file"]["name"])) {
			...
			if ($Main_Size > 1000 || $Second_Size > 1000) {
				notification("Images must be under 500KB!", "/themes?upload", "red");
				exit();
			}

			if ($CSS_Size > 50) {
				notification("The CSS File must be under 25KB!", "/themes?upload", "red");
				exit();
			}
			...
			if (!$Uploader->processed) {
				notification("Something went wrong!", "/themes?upload", "red");
				exit();
			}
			...
			if (strtolower($CSS_Type) == "css") {
				...
				notification("You must upload a .css file!", "/themes?upload", "red");
				exit();
			}
		...
		$DB->modify(
			"INSERT INTO themes VALUES(:URL, :TITLE, :DESCRIPTION, :CATEGORY, :LOGGED, :HEADER, :CHROME, :FIREFOX, :EDGE, :INTERNET, :OPERA, :UPLOADER, NOW(), 0, 0)",
			[
				":URL" => $URL,
				":TITLE" => $Title,
		...
		if ($DB->RowNum == 1) {
			notification("Theme successfully submitted. Now it just needs to be approved!", "/themes", "green");
			exit();
		}
...
if (isset($_GET["a"]) && strlen($_GET["a"]) < 30) {
	...
	if ($_THEMES->has_installed_theme($_GET["a"])) {
		...
		if (count($_THEMES->Themes) > 0) {
			...
			notification("Theme got successfully uninstalled!", "/themes?your=1", "green");
			exit();

		} else {

			notification("Theme got successfully uninstalled!", "/themes", "green");
			exit();
		...
		notification("Theme got successfully installed!", "/themes?your=1", "green");
		exit();
...
if (isset($_GET["accept"]) && ($_USER->Is_Mod || $_USER->Is_Admin)) {
	...
	notification("Theme successfully accepted!", "/themes", "green");
	exit();
}
...
if (!isset($_GET["t"])) {
	...
	if (isset($_GET["search"]) && !empty($_GET["search"]) && mb_strlen($_GET["search"]) <= 128) {
		...
		if ($DB->RowNum == 0) {
			notification("No themes could be found!", "/themes");
			exit();
		}
	} else {
			...
			if ($Delete_Theme["owner"] == $_USER->username || $_USER->Is_Admin || $_USER->Is_Mod) {
				...
				if ($DB->RowNum == 1) {
					...
					if ($_USER->username == $Delete_Theme["owner"]) {
						notification("Theme successfully deleted!", "/themes?your=1", "green");
						exit();
					} else {
						notification("Theme successfully deleted!", "/themes", "green");
						exit();
					}
				} else {
