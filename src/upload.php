...
if ($DB->execute("SELECT value FROM settings WHERE name = 'uploader'", true)["value"] == 0) {
    notification("The uploader has been temporarily disabled!", "/");
    exit();
}
...
if ($Videos_Today >= $Max_Daily_Videos) {
    notification("You cannot upload more than $Max_Daily_Videos videos in a 24-hour period! Sorry about that.", "/", "red");
    exit();
}
