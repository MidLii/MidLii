...
if ($Videos_enabled == 0) {
    notification("The video page has been temporarily disabled!", "/");
    exit();
}
...
if ($_VIDEO->exists() !== false) {
    ...
    if ($_VIDEO->Info["privacy"] == 2 && (!$_USER->logged_in || $_USER->username != $_VIDEO->Info["uploaded_by"])) {
        ...
        notification("You cannot watch this private video!", "/videos", "red");
        exit();
    ...
    if ($_USER->logged_in && ($_VIDEO->Info["s_comments"] == 2)) {
        ...
        $DB->execute(
            "SELECT id FROM friends WHERE ((friend_1 = :USERNAME AND friend_2 = :UPLOADER) OR (friend_1 = :UPLOADER AND friend_2 = :USERNAME)) AND status = 1",
            false,
            [
    ...
    if ($_USER->logged_in) {
        ...
        //CHECK IF BLOCKED
        $Blocked = $DB->execute(
            "SELECT blocker, blocked FROM users_block WHERE (blocker = :USERNAME AND blocked = :UPLOADER) OR (blocker = :UPLOADER AND blocked = :USERNAME)",
            true,
            [
                ":USERNAME" => $_USER->username,
    ...
    notification("This video doesn't exist!", "/", "red");
    exit();
}
